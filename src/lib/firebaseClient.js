...
export function getFirebaseAuthErrorMessage(error) {
  ...
  const messages = {
    ...
    'permission-denied': '云端保存被 Firestore Rules 拒绝。请在 Firebase Console → Firestore Database → Rules 发布仓库最新 firestore.rules 后重试；动态图片不需要 Firebase Storage。',
    ...
    'post-image-too-large': '图片压缩后仍然过大，请选择更小的图片再试。',
